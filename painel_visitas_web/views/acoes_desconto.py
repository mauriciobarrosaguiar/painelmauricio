...
def render_acoes_desconto(inventario: pd.DataFrame, action_records: list[dict] | None = None):
    ...
    st.caption("Campanhas do tipo melhor preco, combo e progressivo por distribuidora.")
    ...
    if inv.empty:
        ...
        st.info("Nenhuma acao com preco promocional imediato foi encontrada para os filtros atuais.")
    else:
        show = pd.DataFrame(
            {
                ...
                "Tipo": vigentes.get("tipo_acao", pd.Series("", index=vigentes.index)).fillna("").astype(str),
                "Acao": vigentes.get("nome_acao", pd.Series("", index=vigentes.index)).fillna("").astype(str),
                ...
                "Cupom": vigentes.get("cupom_acao", pd.Series("", index=vigentes.index)).astype(str),
                "Validade": vigentes.get("validade_acao", pd.Series("", index=vigentes.index)).astype(str).map(_format_date),
            }
        )
