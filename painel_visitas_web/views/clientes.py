...
def _safe_int(v, default=0):
    ...
    return int(default if pd.isna(n) else n)


def _safe_float(v, default=0.0):
    n = pd.to_numeric(v, errors='coerce')
    return float(default if pd.isna(n) else n)
    ...
    for _, row in df.iterrows():
        itens.append({
            **cabecalho,
            ...
            'Preço': _safe_float(row.get('preco_sem_imposto', 0), 0.0),
...
def _choose_df(title: str, df: pd.DataFrame, cabecalho: dict, key_prefix: str):
    ...
    edited = st.data_editor(show, hide_index=True, use_container_width=True, num_rows='fixed', height=min(300, 36 * (len(show) + 1)), key=f'editor_{key_prefix}', column_config={'Sel.': st.column_config.CheckboxColumn('Sel.'), 'Qtde': st.column_config.NumberColumn('Qtde', min_value=1, step=1), 'EAN': None})
    ...
    one = c2.selectbox('Adicionar um por um', ['Escolha a opção'] + work['principio_ativo'].tolist(), key=f'one_{key_prefix}')
    if one != 'Escolha a opção' and c2.button('Adicionar item', key=f'btn_one_{key_prefix}', use_container_width=True):
        row = work[work['principio_ativo'] == one].head(1)
        _add_to_cart(_build_cart_items(row, cabecalho))
        st.success('Produto adicionado ao carrinho.')


def render_clientes(score_df: pd.DataFrame, oportunidades: pd.DataFrame, cancelados: pd.DataFrame, base_full: pd.DataFrame, produtos: pd.DataFrame, inventario: pd.DataFrame, foco: pd.DataFrame | None = None, clientes_df: pd.DataFrame | None = None):
    ...
    st.caption(f'Total de clientes no painel: {len(df)}')
    c1, c2, c3, c4 = st.columns([1.1, 1.0, .9, .9])
    status_mes = c1.selectbox('Compra no mês', ['Todos', 'Com compra no mês', 'Sem compra no mês'])
    potencial = c2.selectbox('Potencial', ['Todos', 'Alto', 'Medio', 'Baixo'])
    sem_prior = c3.toggle('Sem prioritário', value=False)
    sem_lanc = c4.toggle('Sem lançamentos', value=False)
    if status_mes == 'Com compra no mês':
        df = df[df['comprou_mes_atual']]
    elif status_mes == 'Sem compra no mês':
        df = df[~df['comprou_mes_atual']]
    if potencial != 'Todos':
    ...
    tabela = df[['nome_fantasia', 'cnpj', 'cidade', 'comprou_mes_atual_label', 'teve_venda_prioritarios', 'teve_venda_lancamentos', 'ol_sem_combate']].copy()
    tabela.rename(columns={'nome_fantasia':'Cliente','cnpj':'CNPJ','cidade':'Cidade','comprou_mes_atual_label':'Compra mês','teve_venda_prioritarios':'Prioritários','teve_venda_lancamentos':'Lançamentos','ol_sem_combate':'OL sem combate'}, inplace=True)
    tabela['Prioritários'] = tabela['Prioritários'].map({True:'Sim', False:'Não'})
    tabela['Lançamentos'] = tabela['Lançamentos'].map({True:'Sim', False:'Não'})
    tabela['OL sem combate'] = tabela['OL sem combate'].map(_money)
    evento = st.dataframe(tabela, use_container_width=True, hide_index=True, on_select='rerun', selection_mode='single-row')
    if df.empty:
        return
    cli_ref = df[['nome_fantasia', 'cnpj']].drop_duplicates().sort_values('nome_fantasia').copy()
    ...
    base_cli = base_full[base_full['cnpj_pdv'].astype(str).isin(rede_cnpjs)].copy() if rede_cnpjs else base_full[base_full['cnpj_pdv'] == cnpj].copy()
    ...
    def _cad(col, default=''):
        ...
        return default
    msg = f"Olá, { _cad('nome_contato','') or nome_cli }"
    wa = _wa_link(_cad('telefone_limpo', _cad('contato','')), msg)
    st.markdown(f"<div class='detail-card'><div class='detail-title'>{_cad('razao_social', nome_cli)}</div><div class='detail-sub'>{_cad('nome_fantasia', nome_cli)} • CNPJ: {cnpj}</div><div class='detail-grid'><div><span>Cidade</span><b>{_cad('cidade','')} - {_cad('uf','')}</b></div><div><span>Endereço</span><b>{_cad('endereco','')}</b></div><div><span>Bairro</span><b>{_cad('bairro','')}</b></div><div><span>Contato</span><b>{_cad('nome_contato','')}</b></div><div><span>Telefone</span><b>{_cad('contato','')}</b></div><div><span>WhatsApp</span><b>{'Disponível' if wa else 'Sem contato'}</b></div></div></div>", unsafe_allow_html=True)
    if wa:
    ...
    if not base_cli.empty:
        ...
        st.markdown(f"<div class='reason-box'>Pedidos: {len(base_cli)} • Faturado: {_money(total_fat)} • Última compra: {pd.to_datetime(base_cli.get('data_do_pedido', pd.Series(dtype='datetime64[ns]')), errors='coerce').max().strftime('%d/%m/%Y') if 'data_do_pedido' in base_cli.columns and not pd.to_datetime(base_cli['data_do_pedido'], errors='coerce').dropna().empty else '—'}</div>", unsafe_allow_html=True)
    comprados = base_cli[base_cli['status_pedido'].isin(['FATURADO', 'FATURADO PARCIAL'])][['ean', 'principio_ativo', 'mix_lancamentos']].drop_duplicates()
    ...
    _choose_df('Prioritários e lançamentos comprados', comprados[['ean','principio_ativo','mix_lancamentos']], cabecalho, f'comp_{cnpj}')
    _choose_df('Prioritários e lançamentos não comprados', nao_comprados[['ean','principio_ativo','mix_lancamentos','distribuidora','preco_sem_imposto','estoque']], cabecalho, f'nc_{cnpj}')
    ...
    _choose_df('Sugestão de Produtos', sug if not sug.empty else pd.DataFrame(columns=['ean','principio_ativo','mix_lancamentos','distribuidora','preco_sem_imposto','estoque']), cabecalho, f'sug_{cnpj}')
    if foco is not None and not foco.empty:
        st.markdown('<div class="section-title">Produtos foco da semana / mês</div>', unsafe_allow_html=True)
        ...
        foco_df['foco'] = True
        _choose_df('Foco em evidência', foco_df[['ean','principio_ativo','mix_lancamentos','distribuidora','preco_sem_imposto','estoque','foco']].drop_duplicates('ean'), cabecalho, f'foco_{cnpj}')
    st.markdown('<div class="section-title">Produtos cancelados</div>', unsafe_allow_html=True)
    canc = cancelados[cancelados['cnpj_pdv'].astype(str).isin(rede_cnpjs)].copy() if (rede_cnpjs and cancelados is not None and not cancelados.empty) else (cancelados[cancelados['cnpj_pdv'] == cnpj].copy() if cancelados is not None and not cancelados.empty else pd.DataFrame())
    if canc.empty:
