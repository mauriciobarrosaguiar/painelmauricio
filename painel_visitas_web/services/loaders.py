...
if st is not None:
    ...
    def _github_bytes(file_name: str, branch: str) -> bytes | None:
        url = f"https://raw.githubusercontent.com/{_repo_owner()}/{_repo_name()}/{branch}/painel_visitas_web/data/{file_name}"
        try:
            resp = requests.get(url, headers=_raw_headers(), timeout=20)
...
def _read_excel_repo_first(path: Path | None) -> pd.DataFrame:
    if path is None:
        return pd.DataFrame()
    raw = _github_bytes(path.name, _repo_branch())
    if raw:
        try:
